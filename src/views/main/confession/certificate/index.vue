...
<template>
    <div class="confession-love">
        <span
            class="love-title"
            :style="{ fontSize: !isMobile ? '30px' : '25px' }"
        >
            !!!喜欢你!! ( >᎑<)♡︎<br />{{ confessionName }}
            {{ confessionName ? "~( >᎑<)♡︎" : "" }}
        </span>
        <img
            class="love-img"
            :src="getStaticFileUrl('img/love/hug.png')"
            alt=""
        />
        <span
            class="love-content"
            :style="{ fontSize: !isMobile ? '25px' : '15px' }"
        >
            💕 从此以后，我们就是彼此最重要的人了 💕
        </span>
        <span
            class="love-name"
            :style="{ fontSize: !isMobile ? '20px' : '10px' }"
        >
            {{ confessingName }} {{ confessingName ? "and" : "love" }}
            {{ confessionName }}
        </span>
    </div>
</template>
