...
export const useMusicPlayer = (audioRef: { value: HTMLAudioElement | null }) => {
  ...
  const currentIndex = computed({
    get: () => Math.min(playerState.value.currentIndex, Math.max(playlist.value.length - 1, 0)),
    set: (value: number) => {
  ...
  const setProgressFromAudio = () => {
    ...
    const nextProgress = duration.value ? (audio.currentTime / duration.value) * 100 : 0;
