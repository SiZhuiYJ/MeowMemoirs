...
<template>
	<div class="register-box">
		...
		<el-input class="rainbow" v-model="rainbowID" style="width: 240px; padding: 15px 0" placeholder="账号"
			clearable />
		<el-input class="phone" v-show="handover === 'userPhone'" v-model="userPhone"
			style="width: 240px; padding: 9px 0" placeholder="电话" clearable />
		<el-input class="email" v-show="handover === 'userEmail'" v-model="userEmail"
			style="width: 240px; padding: 9px 0" placeholder="邮箱" clearable />
		...
		<el-radio-group v-model="handover" @change="handoverLogin" fill="#a262ad80" text-color="#FFF">
			<el-radio-button v-for="(item, index) in handoverList" :key="index" :label="item.label"
				:value="item.value" />>
		</el-radio-group>
