<script setup lang="ts">
  ...
  onMounted(async () => {
    ...
    meowNoticeSuccess(`
      IP地址:${ accessStore.SimpleIP?.ip || '未知' }<br>
      AS编号:${ accessStore.SimpleIP?.as?.number || '未知' }<br>
      AS名称:${ accessStore.SimpleIP?.as?.name || '未知' }<br>
      运营商:${ accessStore.SimpleIP?.as?.info || '未知' }<br>
      地址段:${ accessStore.SimpleIP?.addr || '未知' }<br>
      国家:${ accessStore.SimpleIP?.country?.name || '未知' }(${ accessStore.SimpleIP?.country?.code || '未知' }<br>
      注册国家:${ accessStore.SimpleIP?.registeredCountry?.name || '未知' }(${ accessStore.SimpleIP?.registeredCountry?.code || '未知' })<br>
      地区:${ (accessStore.SimpleIP?.regions || []).join(' / ') || '未知' }<br>
      地区简称:${ (accessStore.SimpleIP?.regionsShort || []).join(' / ') || '未知' }<br>
      连接类型:${ accessStore.SimpleIP?.type || '未知' }<br>
      `, "欢迎使用喵喵系统", 4000, "success", true);
  });
