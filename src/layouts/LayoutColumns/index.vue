...
const menuList = computed(() =>
  showMenuList.filter((item: AppRouteRecordRaw) => item.meta.isHide == "1")
);
...
watch(
  ...
  () => {
    ...
    const menuItem: AppRouteRecordRaw[] = menuList.value.filter(
      (item: AppRouteRecordRaw) => {
        // 刷新浏览器，一级路由就会变成点击的二级路由，所以需要加上`/${route.path.split("/")[1]}` 进行获取，否则就没有默认选择的颜色。
        return (
          route.path === item.path ||
          route.path.split("/").slice(0, 3).join("/") === item.path
        );
    ...
    if (!menuItem[0]?.children || !menuItem[0].children?.length)
    ...
    <div class="layout-column">
      <el-scrollbar>
        <div
          ...
          :class="{
            'is-active':
              columnActive === item.path ||
              columnActive.split('/').slice(0, 3).join('/') === item.path,
          }"
          @click="handleSubMenu(item)"
          :title="columnActive + '+' + item.path"
        >
