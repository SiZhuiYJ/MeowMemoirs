...
const TIME_UPDATE_THROTTLE = 200
...
// 新增视频源切换处理
watch(() => props.showdata.MediaContent[0], (newSrc) => {
    resetVideoState()
...
<template>
    ...
    <transition name="fade">
        <div v-show="modelValue" class="show-data-container" ref="containerRef" tabindex="0" @keydown="handleKeyboard"
            @mousemove="resetControlsTimer" @mouseleave="hideControls">
            <!--头 -->
            ...
            <div class="show-data" v-if="showdata.type === 'video'">
                ...
                <!-- 视频元素 -->
                <video ref="videoRef" :src="showdata.MediaContent[0]" :style="videoTransform"
                    :poster="showdata.coverImage" :autoplay="autoplay" :preload="preload" @click="togglePlay"
                    @dblclick="toggleFullscreen" @play="isPlaying = true" @pause="isPlaying = false"
                    @timeupdate="handleTimeUpdate" @progress="handleProgress" @loadedmetadata="handleLoadedMetadata"
                    class="video-content" @readystatechange="handleReadyStateChange"></video>
            </div>
            <!-- 轮播组件 -->
            <el-carousel v-else-if="showdata.type === 'atlas'" :interval="5000" arrow="hover" class="show-data"
                :indicator-position="showControls ? '' : 'none'" :autoplay="isPlaying" @change="handleChange">
                <el-carousel-item v-for="(u, i) in showdata.MediaContent" :key="i">
                    <!-- 图片容器 -->
                    <div class="show-backdrop" style="">
                        <img :src="u" alt="" srcset="">
                    </div>
                    <img class="show-img" :src="u" :alt="`展示图 ${i + 1}`" srcset="" :style="videoTransform">
                </el-carousel-item>
            </el-carousel>
            <!-- 关闭按钮 -->
            <transition name="fade">
                <button v-show="isNeat" @click="handleClose" class="close-btn" aria-label="关闭展示">
                    <el-icon>
                        <ArrowLeft />
                    </el-icon>
                </button>
            ...
            <transition name="fade">
                <div v-show="showControls" class="controls">
                    ...
                    <div class="progress-bar" v-show="showdata.type === 'video'">
                        ...
                        <input type="range" min="0" :max="duration" :value="currentTime" @input="handleSeek"
                            aria-label="视频进度" class="seek-bar" />
                    </div>
                    ...
                    <div class="control-group">
                        ...
                        <button @click="togglePlay" :aria-label="isPlaying ? '暂停' : '播放'">
                            <i class="cat-icon" v-show="isPlaying">
                                <svg t="1740803249511" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                    xmlns="http://www.w3.org/2000/svg" p-id="1315" width="30" height="30">
                                    <path
                                        ...
                                        fill="#FFFFFF" p-id="1316"></path>
                                </svg>
                            </i>
                            <i class="cat-icon" v-show="!isPlaying">
                                <svg t="1740803239414" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                    xmlns="http://www.w3.org/2000/svg" p-id="1149" width="30" height="30">
                                    <path
                        ...
                        <div class="volume-control">
                            <button @click="toggleVolumePanel" :aria-label="isMuted ? '取消静音' : '静音'">
                                <svg t="1741082093466" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                    v-if="isMuted || volume === 0" xmlns="http://www.w3.org/2000/svg" p-id="8025"
                                    width="20" height="20">
                                    <path
                                        ...
                                        fill="#FFFFFF" p-id="8026"></path>
                                </svg>
                                <svg t="1741082233623" class="icon" viewBox="0 0 1024 1024" version="1.1" v-else
                                    xmlns="http://www.w3.org/2000/svg" p-id="9083" width="20" height="20">
                                    <path
                        ...
                        <div class="playback-rate" v-show="showdata.type === 'video'">
                            <select v-model.number="playbackRate" aria-label="播放速度" @change="resetControlsTimer">
                        ...
                        <div class="clear-screen">
                            <el-switch v-model="isNeat" inline-prompt active-text="显示" inactive-text="清屏"
                                size="small" />
                        </div>
                        <!-- 旋转 -->
                        <div class="transform-controls">
                            <button @click="toggleTransformControls">
                                <svg t="1741700723396" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                    xmlns="http://www.w3.org/2000/svg" p-id="4835" width="20" height="20">
                                    <path
                                        ...
                                        fill="#FFFFFF" p-id="4836"></path>
                                </svg>
                            </button>
                            <transition name="fade">
                                <div v-show="isTransformControls" class="transform-bar">
                                    <button @click="rotateVideo(90)" aria-label="顺时针旋转90度">
                                        <svg t="1741074833423" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" p-id="4037" width="20" height="20">
                                            <path
                                            ...
                                            <path
                                                ...
                                                fill="#FFFFFF" p-id="4039"></path>
                                        </svg>
                                    </button>
                                    <button @click="rotateVideo(-90)" aria-label="逆时针旋转90度">
                                        <svg t="1741075052707" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" p-id="5023" width="20" height="20">
                                            <path
                                            ...
                                            <path
                                                ...
                                                fill="#FFFFFF" p-id="5025"></path>
                                        </svg>
                                    </button>
                                    <button @click="toggleFlip('horizontal')" aria-label="水平翻转">
                                        <svg t="1741075120287" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" p-id="6038" width="20" height="20">
                                            <path d="M252.76928 299.904l146.2784 0 0 472.42752-146.2784 0 0-472.42752Z"
                                                fill="#FFFFFF" p-id="6039"></path>
                                            <path d="M477.48096 85.34528l70.87104 0 0 885.80608-70.87104 0 0-885.80608Z"
                                            ...
                                            <path
                                                ...
                                                fill="#FFFFFF" p-id="6041"></path>
                                        </svg>
                                    </button>
                                    <button @click="toggleFlip('vertical')" aria-label="垂直翻转">
                                        <svg t="1741075160297" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                            xmlns="http://www.w3.org/2000/svg" p-id="7046" width="20" height="20">
                                            <path d="M286.01856 250.91584l472.4224 0 0 146.2784-472.4224 0 0-146.2784Z"
                                                fill="#FFFFFF" p-id="7047"></path>
                                            <path d="M87.19872 475.62752l885.80096 0 0 70.87104-885.80096 0 0-70.87104Z"
                                                fill="#FFFFFF" p-id="7048"></path>
                                            <path
                        ...
                        <!-- 全屏 -->
                        <button @click="toggleFullscreen" :aria-label="isFullscreen ? '退出全屏' : '全屏'">
                            <svg t="1741074200936" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                xmlns="http://www.w3.org/2000/svg" p-id="4355" width="30" height="30"
                                v-show="!isFullscreen">
                                <path
                                ...
                                <path
                                    ...
                                    fill="#FFFFFF" p-id="4357"></path>
                            </svg>
                            <svg t="1741074323184" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                xmlns="http://www.w3.org/2000/svg" p-id="6984" width="30" height="30"
                                v-show="isFullscreen">
                                <path
            ...
            <transition name="fade">
                <div v-show="isNeat" class="show-right-btn">
                    ...
                    <div class="btn" @click="handleIsLike" title="点赞">
                        <svg t="1740804394565" class="icon" viewBox="0 0 1024 1024" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" p-id="4825" width="30" height="30">
                            <path
                                ...
                                :fill="dataList.isLike ? 'red' : '#FFFFFF'" p-id="4826"></path>
                        </svg>
                        <span :style="{ color: dataList.isLike ? 'red' : '#FFFFFF' }">{{ dataList.like }}</span>
                    </div>
                    <div class="btn" @click="handleisComment" title="评论">
                        <svg t="1740804720171" class="icon" viewBox="0 0 1024 1024" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" p-id="6246" width="30" height="30">
                            <path
                                ...
                                fill="#FFFFFF" p-id="6247"></path>
                        </svg>
                        <span>{{ dataList.comment }}</span>
                    </div>
                    <div class="btn" @click="handleIsCollect" title="收藏">
                        <svg t="1740804432669" class="icon" viewBox="0 0 1024 1024" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" p-id="4975" width="30" height="30">
                            <path
                                ...
                                :fill="dataList.isCollect ? '#FFB802' : '#FFFFFF'" p-id="4976"></path>
                        </svg>
                        <span :style="{ color: dataList.isCollect ? '#FFB802' : '#FFFFFF' }">{{ dataList.collect
                            }}</span>
                    </div>
                    <div class="btn" @click="handleIsShare" title="转发">
                        <svg t="1740804860675" class="icon" viewBox="0 0 1024 1024" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" p-id="4533" width="30" height="30">
                            <path
                    ...
                    <div class="btn more-btn" @click="handleClose" title="更多">
                        <svg t="1740804582209" class="icon" viewBox="0 0 1024 1024" version="1.1"
                            xmlns="http://www.w3.org/2000/svg" p-id="2495" width="30" height="30">
                            <path d="M288 512m-64 0a64 64 0 1 0 128 0 64 64 0 1 0-128 0Z" fill="#ffffff" p-id="2496">
                            </path>
                            <path d="M512 512m-64 0a64 64 0 1 0 128 0 64 64 0 1 0-128 0Z" fill="#ffffff" p-id="2497">
                            </path>
                            <path d="M736 512m-64 0a64 64 0 1 0 128 0 64 64 0 1 0-128 0Z" fill="#ffffff" p-id="2498">
                            </path>
                        </svg>
                    </div>
                </div>
        ...
        .progress-bar {
            ...
            .seek-bar {
                ...
                &::-webkit-slider-thumb {
                    ...
                    width: 32px;
                    height: 16px;
                    background: url('https://localhost:5219/Gallery/catsdiary.jpg') center/32px no-repeat;
                    border-radius: 10px;
