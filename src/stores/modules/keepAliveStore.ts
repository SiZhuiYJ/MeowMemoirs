...
export const useKeepAliveStore = defineStore(
  ...
  () => {
    ...
    const addKeepAliveName = async (name: string) => {
      !keepAliveStore.value.keepAliveName.includes(name) &&
        keepAliveStore.value.keepAliveName.push(name);
    };
    ...
    const removeKeepAliveName = async (name: string) => {
      keepAliveStore.value.keepAliveName =
        keepAliveStore.value.keepAliveName.filter(
