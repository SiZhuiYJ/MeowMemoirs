...
function PenaltySet() {
  ...
  function getLoveImgUrl(name: string) {
    return new URL(
      "/File/MediaFile/LoveCalendar/MapStorage?path=" + name,
      rootUrl
    ).href;
  }
  //系统照片
  function getLocalImgUrl(name: string) {
    return new URL(
      "/File/MediaFile/indigenous/MapStorage?path=" + name,
      rootUrl
    ).href;
  }
  //用户头像
  function DownloadUserImgUrl(data: image) {
    return new URL(
      `/File/MediaFile/${data.RainbowID}/MapStorage?path=${
  ...
  function getHouseUserImgUrl(data: image) {
    return new URL(
      `/File/MediaFile/indigenous/MapStorage?path=${
        data.imageName.split(".")[0]
      }.webp`,
      rootUrl
    ).href;
  ...
  function getMessageImgUrl(data: image) {
    return new URL(
      `/File/MediaFile/${data.RainbowID}/MapStorage?path=messageImage/${
        data.imageName.split(".")[0]
      }.webp`,
      rootUrl
    ).href;
  ...
  function getImgUrl(data: image) {
    return new URL(
      `/File/MediaFile/indigenous/MapStorage?path=image/2- (${data}).jpg`,
      rootUrl
    ).href;
  }
  //Gallery
